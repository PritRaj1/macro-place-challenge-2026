...
)  # project root (for macro_place.*)
sys.path.insert(
    0, str(Path(__file__).parent.parent / "macro_place")
)  # for direct imports
...
def run_orfs_flow(
    ...
    final_artifacts = (
        list(results_dir.glob("6_final.*")) if results_dir.exists() else []
    )
    ...
    if result.returncode != 0:
        print(
            f"WARNING: ORFS exited with code {result.returncode} but final artifacts exist — parsing metrics anyway"
        )
...
def parse_orfs_results(flow_dir: Path, tech: str, design_name: str) -> dict:
    ...
    try:
        ...
        cmd = [
            ...
            tech,
            "--logs",
            f"logs/{tech}/{nickname}/base",
            "--reports",
            f"reports/{tech}/{nickname}/base",
            "--results",
            f"results/{tech}/{nickname}/base",
            "--output",
            str(metrics_file),
        ]
...
def evaluate_benchmark(
    ...
    source_dir_overrides = {
        "bp_quad": Path(
            "external/MacroPlacement/CodeElements/SimulatedAnnealingGWTW/test/bp_ng45"
        ),
    }
    ...
    if source_name in source_dir_overrides:
        ...
        source_dir = Path(
            f"external/MacroPlacement/Flows/NanGate45/{source_name}/netlist/output_CT_Grouping"
        )
    else:
        source_dir = Path(
            f"external/MacroPlacement/Flows/ASAP7/{source_name}/netlist/output_CT_Grouping"
        )
    ...
    if placement_path is not None:
        ...
        print(
            f"✓ Loaded placement from {placement_path} (shape: {list(placement.shape)})"
        )
    else:
    ...
    if tech == "nangate45":
        orfs_config_dir = Path(
            f"external/MacroPlacement/Flows/NanGate45/{source_name}/scripts/OpenROAD/{source_name}"
        )
    else:
        orfs_config_dir = Path(
            f"external/MacroPlacement/Flows/ASAP7/{source_name}/scripts/OpenROAD/{source_name}"
        )
    ...
    if not orfs_config_dir.exists():
        ...
        for tar_path in tar_parent.glob("*.tar.gz") if tar_parent.exists() else []:
            ...
            with tarfile.open(tar_path) as tar:
                ...
                config_members = [
                    m for m in tar.getmembers() if m.name.endswith("config.mk")
                ]
                if config_members:
                    ...
                    print(
                        f"  ✓ Extracted {len(tar.getmembers())} files to {orfs_config_dir}"
                    )
                    break
...
""")
    ...
    if orfs_config_dir.exists():
        ...
        if tech == "asap7":
            ...
            if asap7_enablements.exists():
                ...
                print(
                    f"  ✓ Copied {len(sram_lefs)} SRAM LEF and {len(sram_libs)} LIB files from Enablements"
                )
        ...
        if config_mk.exists():
            ...
            if genus_netlist_dir.exists():
                for candidate in sorted(genus_netlist_dir.glob("*.v")):
                    ...
                    if n_sram > 0:
                        ...
                        lzc_patch_file = (
                            Path(__file__).parent / "ariane133_lzc_patches.v"
                        )
                        if lzc_patch_file.exists():
                            ...
                            if needed:
                                with open(patched_netlist, "a") as pf:
                                    ...
                                    pf.write(patch_text)
                                print(
                                    f"  ✓ Patched {len(needed)} missing modules: {', '.join(needed)}"
                                )
            ...
            if source_name == "mempool_tile":
                ...
                print(
                    "  ✓ Fixed mempool_tile config (disabled hierarchical flow, increased die to 2000x2000, opened all pin sides)"
                )
        ...
        m = re.search(
            r"CORE_AREA\s*=\s*([\d.]+)\s+([\d.]+)\s+([\d.]+)\s+([\d.]+)", config_text
        )
        if m:
        ...
        write_orfs_macro_placement(
            ...
            str(tcl_file),
            core_area=core_area,
            use_genus_names=_using_genus_netlist,
        )
    ...
    if config_path.exists():
        ...
        if m:
            nickname = m.group(1)
    stale_names = (
        {orfs_design_name, nickname} if orfs_config_dir.exists() else {benchmark_name}
    )
...
def main():
    ...
    for result in all_results:
        ...
        wire_str = (
            f"{wire_length / 1e6:.2f}"
            if isinstance(wire_length, (int, float))
            else "N/A"
        )
