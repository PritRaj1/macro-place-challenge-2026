...
def write_orfs_macro_placement(
    placement, benchmark, plc, output_file, core_area=None, use_genus_names=False
):
    """
    ...
    if core_area is not None:
        ...
        print(
            f"  Center offset: ({offset_x:.1f}, {offset_y:.1f}) — no scaling, preserving distances"
        )
    ...
    if core_area is not None and MIN_GAP <= 0:
        print(
            f"  ORFS_MIN_GAP_UM={MIN_GAP} — spacing push disabled, "
    ...
    if core_area is not None and MIN_GAP > 0:
        ...
        for gp in group_data:
            for mk in group_data[gp]:
                ...
                if node:
                    _all.append(
                        ("group", gp, mk, x, y, node.get_width(), node.get_height(), pn)
                    )
        for idx, (odb_n, x, y, orient, pn) in enumerate(direct_placements):
            ...
            if node:
                _all.append(
                    ("direct", idx, None, x, y, node.get_width(), node.get_height(), pn)
                )
        ...
        for entry in _all:
            ...
            if mag > 1e-6:
                moved_rows.append(
                    (pn, x_before, y_before, x_after, y_after, dx, dy, mag)
                )
        ...
        if moved_rows:
            ...
            with open(diff_path, "w") as dfh:
                dfh.write(
                    f"# Post-push spacing adjustment (Tier 2 only, MIN_GAP={MIN_GAP}μm)\n"
                )
                dfh.write(
                    f"# Submitted placement had {len(moved_rows)}/{len(_all)} macros "
                    f"with <{MIN_GAP}μm clearance to a neighbour; pushed apart below.\n"
                )
                dfh.write(
                    "# Columns: macro  x_before  y_before  x_after  y_after  dx  dy  |Δ|\n"
                )
                for pn, xb, yb, xa, ya, dx, dy, mag in moved_rows:
                    dfh.write(
                        f"{pn}\t{xb:.3f}\t{yb:.3f}\t{xa:.3f}\t{ya:.3f}\t"
                        f"{dx:+.3f}\t{dy:+.3f}\t{mag:.3f}\n"
                    )
            max_disp = moved_rows[0][7]
            mean_disp = sum(r[7] for r in moved_rows) / len(moved_rows)
            print(
                f"  ✓ Spacing enforcement: {iteration + 1} iters, {MIN_GAP}μm min gap, "
    ...
    with open(output_file, "w") as f:
        ...
        for group_prefix in sorted(group_data.keys()):
            for k in sorted(group_data[group_prefix].keys()):
                ...
                f.write(
                    f'dict set _gpd "{group_prefix}" {k} [list {x_ll:.6f} {y_ll:.6f} {orient} "{plc_name}"]\n'
                )
        f.write("\n")
...
""")
        ...
        if direct_placements:
            ...
            for odb_name, x_ll, y_ll, orient, plc_name in direct_placements:
                ...
                f.write("    set _grid [[ord::get_db_tech] getManufacturingGrid]\n")
                f.write(
                    "    if {$_grid > 0} { set _x [expr {($_x / $_grid) * $_grid}]; set _y [expr {($_y / $_grid) * $_grid}] }\n"
                )
                f.write(f"    $_inst setOrient {orient}\n")
                f.write("    $_inst setLocation $_x $_y\n")
                f.write("    $_inst setPlacementStatus FIRM\n")
                f.write("    incr _placed\n")
                f.write("} else {\n")
        ...
        f.write(
            f'    puts "WARNING: Expected {total_macros} macros but placed $_placed"\n'
        )
    ...
    print(
        f"  {total_macros} macros ({n_genblk} sram groups, {n_direct} direct placements)"
    )


def main():
    parser = argparse.ArgumentParser(
        description="Generate OpenROAD macro placement TCL"
    )
    parser.add_argument(
        "--benchmark", default="ariane133", help="Benchmark to use (default: ariane133)"
    )
    parser.add_argument(
        "--output",
        default="output/place_macros.tcl",
    ...
    # Write TCL script
    print("Writing OpenROAD TCL script...")
